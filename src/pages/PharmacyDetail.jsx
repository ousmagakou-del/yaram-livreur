...
export default function PharmacyDetail({ pharmacyId }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const list = [];
      (inv || []).forEach(i => {
        if (i.products && i.products.id) {
          list.push({ ...i.products, stock: i.stock });
        }
      });
      
      console.log('PharmacyDetail: loaded', list.length, 'products');
      setProducts(list);
      setLoading(false);
    })();
  }, [pharmacyId]);
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            {displayed.map((p, idx) => (
              ...
              >
                ...
                />
                {p.brand && (
                  <div style={{ fontSize: 10, color: '#6B6B6B', fontWeight: 600, textTransform: 'uppercase' }}>
                    {p.brand}
                  </div>
                )}
                <div style={{
